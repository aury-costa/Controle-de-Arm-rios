...
// ================== STATE ==================
let config = { totalArmarios: 300, baixoAte: 150, splitAte: 150, splitTipo: "CIMA" };
...
const cfgSplitTipo = $("cfgSplitTipo");
...
function getPosicaoDefault(num) {
  const n = Number(num);
  return (n<=Number(config.baixoAte||0)) ? "BAIXO" : "CIMA";
}
...
btnSalvarConfig.addEventListener("click", async ()=>{
  const total = Math.max(1, Number(cfgTotal.value||300));
  const ate = Math.max(0, Number(cfgBaixoAte.value||0));
  const tipo = (cfgSplitTipo && cfgSplitTipo.value) ? cfgSplitTipo.value : (config.splitTipo || "CIMA");

  const payload = {
    totalArmarios: Math.floor(total),
    splitAte: Math.floor(ate),
    splitTipo: (tipo === "BAIXO" ? "BAIXO" : "CIMA"),
    // legado (compat com versões antigas)
    baixoAte: Math.floor(ate)
  };
...
onValue(ref(db, "config"), (snap)=>{
  ...
  // --- compat + flex: splitAte/splitTipo ---
  if (config.splitAte == null) config.splitAte = (config.baixoAte != null ? config.baixoAte : 150);
  if (!config.splitTipo) config.splitTipo = "CIMA";
  if (cfgBaixoAte) cfgBaixoAte.value = config.splitAte;
  if (cfgSplitTipo) cfgSplitTipo.value = config.splitTipo;
